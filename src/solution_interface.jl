...
Generates the canned visualization of name `name` for a given `AbstractAnalysisSolution`.
The `name` must be one of the allowed names in the
`AnalysisSolutionMetadata(sol).artifacts`. The result of this
function should match the corresponding `CannedVisualizationType` declared in the
AnalysisSolutionMetadata.
"""
